...
import { IMsalConfig, AuthProvider } from './authProvider';
import {
    Ipc_Log,
    Ipc_MsalConfig,
    Ipc_Signin,
...
ipcMain.handle(Ipc_GetProfile, async (_event: IpcMainInvokeEvent): Promise<any> => {
    ...
    const graphResponse = await authProvider.callEndpointWithToken(`${store.get(StoreKeys.graphEndpointHost)}${store.get(StoreKeys.graphMeEndpoint)}`, token);
